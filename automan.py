...
class AutoMan:
    def __init__(self, baseDir):
        self.defaultRetry = 0                           # 默认一直重试
        self.defaultRepeat = 1                          # 默认重复一次
        ...
        self.commandsMap = {                            # 当前支持的指令
            '点击': {"type": "click", "udlr": 'left', 'times': 1,},
            '移动': {"type": "click", "udlr": 'left', 'times': 0,},
            '单击': {"type": "click", "udlr": 'left', 'times': 1,},
            '三击': {"type": "click", "udlr": 'left', 'times': 3,},
            '双击': {"type": "click", "udlr": 'left', 'times': 2,},
            '输入': {"type": "input", "value": "", },
            '上滚': {"type": "scroll", "step": 150, },
            '下滚': {"type": "scroll", "step": -150, },
            '按键': {"type": "hotkey", "value":"" },
            '等待': {"type": "wait", "value":""},
            '时钟': {"type": "clock", "value":""},
            "标记": {"type": "tag", "value":""},
            "回到": {"type": "goto", "value":""},
    ...
    def parseCommand(self):
        ...
        for line in lines:
            ...
            if task['type'] == 'click':
                ...
                images = []
                for name in names:
                    images.append(self.fetchImage(name))
                task['value'] = images
    ...
    def executeCommand(self,tasks):
        ...
        while self.line < len(tasks):
            ...
            def doTask():
                ...
                if task['type'] == 'click':
                    if 'duration' in task['ext'] and len(task['ext']['duration']):
                        self.mouseDuration = float(task['ext']['duration'].pop())
                    ...
                    if tag in self.tagMap:
                        ...
                        showError('第%i行跳转的位置输入有误'%(self.line+1) )
    ...
    def fetchImage(self, name):
        ...
        self.showError("找不到图片，请确保【%s】图片存在"% path)
